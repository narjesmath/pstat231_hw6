...
```
From the correlation plot, we can see that the variables special defense and defense have the highest correlation.Other variables that have pretty high correlation include attack and defense and special attack and special defense. 
...
ROC ACU peaks at around 400 trees and then slowly declines again.
...
The best model is the random forest model, with an AUC ROC of 0.727.

Print the AUC value of your best-performing model on the testing set. Print the ROC curves. Finally, create and visualize a confusion matrix heat map.

```{r}
rf_test_fit = augment(rf_fit, new_data = poke_test) #get predictions for test data
roc_auc(rf_test_fit, truth = type_1, .pred_Bug, .pred_Fire, .pred_Grass, .pred_Normal, .pred_Water, .pred_Psychic) #print ROC AUC for test data
```

```{r}
autoplot(roc_curve(rf_test_fit, truth = type_1, .pred_Bug, .pred_Fire, .pred_Grass, .pred_Normal, .pred_Water, .pred_Psychic)) #plot ROC curves for each class
```
...
The RMSE for the testing data set is 2.10